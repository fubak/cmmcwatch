#!/usr/bin/env python3
"""Articles index page renderer for CMMC Watch.

``render_articles_index`` was extracted verbatim from
``EditorialGenerator.generate_articles_index``. It renders the searchable /
filterable index of all editorial articles. It embeds today's date via
datetime.now() in the shared header/footer (matching the original behaviour),
so the page is otherwise a pure function of its inputs.
"""
...
def render_articles_index(
    ...
    """Render the searchable articles index page (client-side filtered).

    Extracted verbatim from EditorialGenerator.generate_articles_index. Embeds
    today's date via datetime.now() in the shared header/footer, matching the
    original behaviour.
    """
