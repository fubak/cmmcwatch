#!/usr/bin/env python3
"""Daily source health checks for CMMC Watch collection endpoints."""
...
def _resolve_headers(source: SourceSpec, domain_profile: Dict[str, Any]) -> Dict[str, str]:
    ...
    if "User-Agent" not in headers:
        headers["User-Agent"] = DEFAULT_BROWSER_UA + " CMMCWatchHealthCheck/1.0"
    ...
    if source.fallback_url:
        try:
            ...
            if fallback_result.get("status") in {"healthy", "degraded"}:
                ...
                result["error"] = f"primary failed: {primary_error}; fallback succeeded"
                return result
